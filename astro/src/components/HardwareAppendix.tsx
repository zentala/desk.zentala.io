          ...
          <div className="parts-grid">
            {parts.map((part) => (
              <div className="parts-row" key={part.name}>
                <img className="parts-thumb" src={part.img} alt={part.name} />
                <div className="parts-name">{part.name}</div>
                <div className="parts-buy">
                  {part.url ? (
                    <a href={part.url} className="btn btn-outline-dark btn-sm" target="_blank" rel="noreferrer">
                      Buy {part.price && <span className="badge text-bg-secondary ms-2">{part.price}</span>}
                    </a>
                  ) : (
